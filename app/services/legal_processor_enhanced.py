...
class EnhancedLegalProcessor:
    ...
    def analyze_comprehensive_legal_document_sync(self, text: str, document_type: str = None) -> Dict[str, Any]:
        """
        Comprehensive legal document analysis (synchronous version)
        """
        try:
            logger.info("Starting comprehensive legal document analysis (sync)")
            
            # Perform multiple analysis layers (all synchronous)
            basic_analysis = self._extract_basic_legal_info(text)
            clause_analysis = self._extract_legal_clauses_sync(text)
            financial_analysis = self._extract_financial_terms(text)
            parties_analysis = self._extract_parties_info(text)
            dates_analysis = self._extract_key_dates(text)
            risk_analysis = self._assess_comprehensive_risk(text, clause_analysis)
            
            # Generate document overview and summary (synchronous versions)
            overview = self._generate_document_overview_sync(text, document_type)
            summary = self._generate_contract_summary_sync(text, clause_analysis, financial_analysis)
            ...
            return {
                ...
                'contract_summary': 'Unable to generate summary due to processing error',
                'main_parties': [{'name': 'No party information found', 'role': 'Unknown', 'type': 'Unknown'}],
                'key_dates': [{'date_type': 'Not Found', 'date_value': 'No key dates identified', 'description': 'No specific dates found', 'importance': 'Low'}],
                'financial_terms': [],
                'legal_clauses': {
                    'termination_clauses': [], 
                    'penalty_clauses': [], 
            ...
            logger.error(f"Error in fallback legal analysis: {e}")
            return {
                'error': f"Analysis failed: {str(e)}",
                'document_overview': {
                    'type': 'Unknown', 
    ...
    def _extract_legal_clauses_sync(self, text: str) -> Dict[str, List[Dict[str, Any]]]:
        """Synchronous version of legal clause extraction"""
        clauses = {
            'termination_clauses': [],
            'penalty_clauses': [],
            'payment_terms': [],
    ...
    def _generate_document_overview_sync(self, text: str, document_type: str = None) -> Dict[str, Any]:
        """Synchronous version of document overview generation"""
        try:
            doc_type = self._classify_document_type(text)
            purpose = self._determine_document_purpose(text, doc_type)
    ...
    def _generate_contract_summary_sync(self, text: str, clauses: Dict, financial_terms: List) -> str:
        """Synchronous version of contract summary generation"""
        try:
            clause_count = sum(len(v) for v in clauses.values() if isinstance(v, list))
            financial_count = len([ft for ft in financial_terms if ft.get('term_type') != 'Not Found'])
    ...
    def _extract_legal_clauses_sync(self, text: str) -> Dict[str, List[Dict[str, Any]]]:
        """Extract and categorize legal clauses (synchronous version)"""
        clauses = {
            'termination_clauses': [],
            'penalty_clauses': [],
            'payment_terms': [],
    ...
    def _generate_document_overview_sync(self, text: str, document_type: str = None) -> Dict[str, Any]:
        """Generate comprehensive document overview (synchronous)"""
        try:
            doc_type = self._classify_document_type(text)
            purpose = self._determine_document_purpose(text, doc_type)
    ...
    def _generate_contract_summary_sync(self, text: str, clauses: Dict, financial_terms: List) -> str:
        """Generate comprehensive contract summary (synchronous)"""
        try:
            clause_count = sum(len(v) for v in clauses.values() if isinstance(v, list))
            financial_count = len([ft for ft in financial_terms if ft.get('term_type') != 'Not Found'])
